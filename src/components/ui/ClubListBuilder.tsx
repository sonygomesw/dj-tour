      ...
      <div className="space-y-8">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="relative">
                <Instagram className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-500 w-4 h-4" />
                <input
                  type="text"
                  value={newClub.instagram}
                  onChange={(e) => setNewClub({ ...newClub, instagram: e.target.value })}
                  placeholder="@clubname"
                  className="w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-12 pr-4 text-gray-900 placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500/50"
                />
              </div>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 bg-purple-500 hover:bg-purple-600 text-gray-900 py-3 px-4 rounded-xl font-medium transition-colors"
          >
        ...
        {clubs.length > 0 && (
          <div className="space-y-4">
            <h3 className="text-lg font-medium text-gray-900">Clubs ajoutés</h3>
            <div className="space-y-3">
              {clubs.map((club) => (
                <div key={club.id} className="flex items-center justify-between bg-white/5 rounded-lg p-4">
                  <div>
                    <h4 className="text-gray-900 font-medium">{club.name}</h4>
                    {club.instagram && (
