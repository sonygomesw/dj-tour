...
interface MissionCarouselProps {
  missions: (Omit<MissionType, "status" | "steps"> & {
    isCompleted?: boolean;
  })[];
  ...
  return (
    <div className="relative">
      ...
      {showLeftArrow && (
        <button
          onClick={() => scroll("left")}
          className="absolute left-2 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full bg-white border border-gray-200 shadow-sm hover:shadow-md transition-all duration-200 flex items-center justify-center"
        >
          {" "}
      ...
      {showRightArrow && (
        <button
          ...
          className="absolute right-2 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full bg-white border border-gray-200 shadow-sm hover:shadow-md transition-all duration-200 flex items-center justify-center"
        >
          {" "}
