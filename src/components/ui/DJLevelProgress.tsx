...
export function DJLevelProgress({
  ...
  const getLevelBorderColor = (level: number) => {
    switch (level) {
      case 1: return 'border-gray-400/30';
      case 2: return 'border-blue-400/30';
      case 3: return 'border-orange-400/30';
      case 4: return 'border-purple-400/30';
      case 5: return 'border-pink-400/30';
  ...
  const getLevelBgColor = (level: number) => {
    switch (level) {
      case 1: return 'bg-gray-500/10';
      case 2: return 'bg-blue-500/10';
      case 3: return 'bg-orange-500/10';
      case 4: return 'bg-purple-500/10';
      case 5: return 'bg-pink-500/10';
  ...
  const getLevelGlowColor = (level: number) => {
    switch (level) {
      case 1: return 'rgba(156, 163, 175, 0.4)';
      case 2: return 'rgba(59, 130, 246, 0.4)';
      case 3: return 'rgba(249, 115, 22, 0.4)';
      case 4: return 'rgba(139, 92, 246, 0.4)';
      case 5: return 'rgba(236, 72, 153, 0.4)';
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center gap-6 mb-8">
            ...
            >
              <div className="w-full h-full rounded-full bg-black/20 backdrop-blur-sm flex items-center justify-center">
                <LevelIcon className={`w-12 h-12 text-gray-900 dark:text-white drop-shadow-lg`} />
              </div>
            </motion.div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-gray-600 dark:text-white/70 text-lg font-medium">
            ...
            <div className="flex justify-between items-center text-sm">
              <span className="text-gray-600 dark:text-white/60">
                {pointsToNext > 0 ? `${pointsToNext.toLocaleString()} points restants` : 'Niveau maximum atteint!'}
              </span>
          ...
          {currentLevel < 5 && (
            ...
            >
              <h4 className="text-gray-700 dark:text-white/80 font-medium mb-2 flex items-center gap-2">
                <Trophy className="w-4 h-4 text-yellow-400" />
                Level {currentLevel + 1} Rewards
              </h4>
              <div className="flex items-center gap-2 mb-2">
                <span className="text-2xl">{getLevelInfo(currentLevel + 1).emoji}</span>
                <span className={`font-bold text-gray-900 dark:text-white`}>
                  {getLevelInfo(currentLevel + 1).name}
                </span>
              </div>
              <ul className="text-gray-600 dark:text-white/60 text-sm space-y-1">
